...
// 		bytechecker = ret_val->num_bytes;
// 		printf("number of bytes in bytechecker: %d\n",bytechecker);
// 		if (ret_val->num_bytes<0) return ret_val;
		if (ret_val->num_bytes<0) {
			ret_val->num_bytes = 0;
// 			return ret_val;
		}
// 		printf("number of bytes2: %d\n",ret_val->num_bytes);
...
//		printf("VD: %d\n",(int)ptr - (int) gcd_ptr->merchant_id);
        ...
        message_min_size = 256+32;
        if (ret_val->num_bytes < message_min_size) return ret_val;
				if (checkval != '\0') break;
